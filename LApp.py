...
class LabelButton(ButtonBehavior, Label, LBase):
	...
	def __init__(self, **kw):
		...
		with self.canvas.before:
			PushMatrix()
			self.rotate = Rotate(angle=180.0,origin=(744.0,299.0))
...
class LOrientation(object):
	...
	def get(self):
		...
		if (self.theta < -45.0):
			return "FLYING"
		if (self.phi < 135.0 and self.phi > 45.0):
			return "BOTTOM"
		if (self.phi < 45.0 and self.phi > -45.0):
			return "LEFT"
		if (self.phi < -45.0 and self.phi > -135.0):
			return "TOP"
		if (self.phi > 135.0 or self.phi < -135.0):
			return "RIGHT"
		# fallback
		return "LANDING"
...
class LWorkWindow(BoxLayout):
    def __init__(self,**kw):
        ...
        with self.canvas.before:
            ...
            Color(0.15, 0.00, 0.3, 1)   # schwarz mit blau stich
            #Color(0, 0.05, 0.1, 1)   # schwarz mit blau-grün stich
            #Color(0, 1, 1, 0.4)
            #Color(0.0, 0.0, 0.99, 1)
        ...
        self.label_phi = LabelButton(text="halloPhi")
        self.label_theta = LabelButton(text="halloTheta")
        self.label_balance = LabelButton(text="Balance")
    ...
    def refresh(self, values):
        ...
        self.values = values
        theta = values.rawTheta
        phi = values.rawPhi
        self.value = theta
        self.angle = phi
        self.ori = LOrientation(phi,theta)
        self.draw_line()
    ...
    def draw_line(self):
        ...
        if self.values is not None:
            ...
            self.value = self.values.rawTheta
            self.angle = self.values.rawPhi
            #t = (90.0 - math.fabs(x)) * msiz / 45.0 / 2.0
            txtangle = 0.0
            if self.ori.get()=="LANDING":
                self.balance = self.value
                self.label_balance.text = str(round(90.0-self.value,1))
                x = self.values.smRoll
                y = -self.values.smPitch
            elif self.ori.get()=="FLYING":
                self.label_balance.text = str(round(-90.0-self.value,1))
                self.balance = self.value
                x = self.values.smRoll
                y = -self.values.smPitch
            elif self.ori.get()=="TOP":
                txtangle = 180.0
                self.balance = norm(self.values.smBalanceX)
                self.label_balance.text = str(round(-self.balance,1))
                x = self.balance
                y = 0
            elif self.ori.get()=="BOTTOM":
                self.balance = norm(self.values.smBalanceX)
                self.label_balance.text = str(round(self.balance,1))
                x = self.balance
                y = 0
            elif self.ori.get()=="LEFT":
                txtangle = -90.0
                self.balance = norm(self.values.smBalanceY)
                self.label_balance.text = str(round(-self.balance,1))
                y = self.balance
                x = 0
            elif self.ori.get()=="RIGHT":
                txtangle = 90.0
                self.balance = norm(self.values.smBalanceY)
                self.label_balance.text = str(round(self.balance,1))
                y = self.balance
        ...
        with self.canvas.after:
            good = 0.3
            if (((math.fabs(self.balance)+good) % 90) < 2.0*good) and self.values is not None:
        ...
        self.label_phi.text = str(round(self.angle,1))
        self.label_theta.text = str(round(self.value,1))
